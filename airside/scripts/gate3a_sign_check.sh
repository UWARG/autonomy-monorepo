...
AIRSIDE_AUTO_ARM=false AIRSIDE_SIM_TARGET=false docker compose up -d || exit 2

echo -n "[3a] waiting for MAVROS<->FCU heartbeat (max 180s)"
for i in $(seq 1 36); do
  if docker compose logs airside 2>/dev/null | grep -q "Got HEARTBEAT"; then
    echo " ... connected"
    break
  fi
  echo -n "."
...
if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
  docker compose logs airside > "${ART}/airside.log" 2>&1
