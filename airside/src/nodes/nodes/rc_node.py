...
class RCNode(Node):
    ...
    def rc_callback(self, msg: Mavlink):
        ...
        if msg.msgid==70:
            try:
                raw = bytes(payload_bytes).ljust(38, b"\x00")
                data = struct.unpack("<8H BB 10H", raw)
            except Exception as e:
                self.get_logger().error(f"msgid: {msg.msgid} Failed to unpack Mavlink message: {e}")
                return
            ...
            try:
                raw = bytes(payload_bytes).ljust(42, b"\x00")
                data=struct.unpack("<I 18H BB", raw)
            except Exception as e:
                self.get_logger().error(f"msgid: {msg.msgid} Failed to unpack Mavlink message: {e}")
                return
            channels=data[1:19]
