...
class AbstractCamera(abc.ABC):
    ...
    def start(self) -> bool:
        """Initializes the camera and starts the background capture thread, returns True on Success."""
        if self._running:
            return True
    ...
    def get_last_frame(self) -> CameraFrame | None:
        """Returns the most recently captured frame, or None if none has been captured yet."""
        with self._frame_lock:
    ...
    def close_camera(self) -> None:
        """Camera specific shutdown logic, releases hardware resources."""
        pass
