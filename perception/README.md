...
```python
from target_location import CameraIntrinsics, GroundPlaneGeometry, locate_target_frd
...
position = locate_target_frd(
    ...
    attitude=GroundPlaneGeometry.from_euler(roll_rad=0.05, pitch_rad=-0.10),
)
# -> TargetPosition(forward_m=..., right_m=..., down_m=...), or None if not localizable
```

`ImageFrame` and `TargetPosition` live in the shared [`utils`](../utils) project; the
camera-geometry types below are local to this module.
...
`GroundPlaneGeometry` reduces the drone's orientation to the only part that affects the
answer: which way is down in the body frame. Heading drops out — the result is in the
body frame, and a level plane looks identical from every heading.

| Constructor | Use when your source gives you |
| --- | --- |
| `GroundPlaneGeometry.from_attitude(attitude)` | A shared `utils.Attitude`, e.g. straight off a MAVLink feed. Only roll and pitch are read. |
| `GroundPlaneGeometry.from_euler(roll_rad, pitch_rad, yaw_rad=0.0)` | Tait-Bryan angles in NED. Yaw is accepted for call-site convenience and ignored. |
| `GroundPlaneGeometry.from_quaternion(w, x, y, z)` | A quaternion rotating **body-FRD → NED**. Immune to gimbal lock. |
| `GroundPlaneGeometry.from_mavros_quaternion(w, x, y, z)` | A `sensor_msgs/Imu` orientation off `/mavros/imu/data`. |
| `GroundPlaneGeometry.level()` | Nothing — a perfectly level drone, mostly for tests. |

`locate_target_frd` also takes a `utils.Attitude` directly, converting it for you.
...
`utils.TargetPosition(forward_m, right_m, down_m)`, with `.to_array()` and
`.range_m`.
...
- **The image is rectified.** Distortion has already been removed, so no distortion
  coefficients are taken. The intrinsics must be those of the rectified image (OpenCV's
  `P` matrix / `camera_info`), not the raw sensor's. If only a field-of-view spec is
  available, use `CameraIntrinsics.from_fov` — it assumes a centred principal point.
