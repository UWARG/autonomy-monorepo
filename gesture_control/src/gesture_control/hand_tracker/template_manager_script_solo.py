...
def send_result_no_hand(pd_inf, nb_lm_inf, sequence_num):
    result = dict([("pd_inf", pd_inf), ("nb_lm_inf", nb_lm_inf)])
    send_result(result, sequence_num)

def send_result_hand(pd_inf, nb_lm_inf, sequence_num, lm_score=0, handedness=0, rect_center_x=0, rect_center_y=0, rect_size=0, rotation=0, rrn_lms=0, sqn_lms=0, world_lms=0, xyz=0, xyz_zone=0):
...
def fit_landmark_rect(sqn_x, sqn_y, sqn_size, rotation):
    """Keep the normalized rotated crop inside the rectangular camera preview."""
    rr_y = (sqn_y * frame_size - pad_h) / img_h
    rr_h = sqn_size * frame_size / img_h
    c = abs(cos(rotation))
    s = abs(sin(rotation))
    # The crop is square in pixels. Convert its extents to each normalized
    # axis before clipping; mixing x and y normalized units distorts it.
    half_x = 0.5 * (c + s) * sqn_size
    half_y = 0.5 * (c + s) * rr_h
    scale = min(1.0, 0.999 / (2 * half_x), 0.999 / (2 * half_y))
    sqn_size *= scale
    rr_h *= scale
    half_x *= scale
    half_y *= scale
    sqn_x = min(max(sqn_x, half_x), 1.0 - half_x)
    rr_y = min(max(rr_y, half_y), 1.0 - half_y)
    return sqn_x, (rr_y * img_h + pad_h) / frame_size, sqn_size
...
while True:
    ...
    if send_new_frame_to_branch == 1: # Routing frame to pd branch
        ...
        pd_result = node.io['from_post_pd_nn'].get()
        sequence_num = pd_result.getSequenceNum()
        detection = pd_result.getLayerFp16("result")
        ...
        if pd_score < ${_pd_score_thresh} or box_size < 0:
            send_result_no_hand(True, 0, sequence_num)
            send_new_frame_to_branch = 1
    ...
    sqn_rr_center_x, sqn_rr_center_y, sqn_rr_size = fit_landmark_rect(
    ...
    sequence_num = lm_result.getSequenceNum()
    ...
    if lm_score > ${_lm_score_thresh}:
        ...
        # Send result to host
        send_result_hand(send_new_frame_to_branch==1, nb_lm_inf, sequence_num, lm_score, handedness, sqn_rr_center_x, sqn_rr_center_y, sqn_rr_size, rotation, rrn_lms, sqn_lms, world_lms, xyz, xyz_zone)
        ...
        send_result_no_hand(send_new_frame_to_branch==1, nb_lm_inf, sequence_num)
        send_new_frame_to_branch = 1
