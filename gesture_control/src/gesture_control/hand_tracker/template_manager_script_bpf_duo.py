...
def add_source_safe_square_transform(config, xmin, ymin, xmax, ymax, output_size):
    """Preserve virtual-square padding without cropping outside the camera frame."""
    size = xmax - xmin
    src_xmin, src_ymin = max(0, xmin), max(0, ymin)
    src_xmax, src_ymax = min(img_w, xmax), min(img_h, ymax)
    if size <= 0 or src_xmax <= src_xmin or src_ymax <= src_ymin:
        return False
    scale = (output_size - 1) / size
    source_points = []
    destination_points = []
    for src_x, src_y in ((src_xmin, src_ymin), (src_xmax - 1, src_ymin),
                         (src_xmax - 1, src_ymax - 1), (src_xmin, src_ymax - 1)):
        source = Point2f()
        source.x, source.y = src_x, src_y
        source_points.append(source)
        destination = Point2f()
        destination.x = (src_x - xmin) * scale
        destination.y = (src_y - ymin) * scale
        destination_points.append(destination)
    config.addTransformFourPoints(source_points, destination_points, False)
    config.setOutputSize(output_size, output_size, ImageManipConfig.ResizeMode.STRETCH)
    return True
...
def fit_landmark_rect(sqn_x, sqn_y, sqn_size, rotation):
    ...
    c = abs(cos(rotation))
    s = abs(sin(rotation))
    half_x = 0.5 * (c * sqn_size + s * rr_h)
    half_y = 0.5 * (s * sqn_size + c * rr_h)
...
while True:
    ...
    if send_new_frame_to_branch == 0: # Routing frame to body detection
        ...
        add_source_safe_square_transform(cfg_pre_body, crop_region['xmin'], crop_region['ymin'], crop_region['xmax'], crop_region['ymax'], ${_body_input_length})
        ...
        if zone:
            ...
            cfg_pre_pd = ImageManipConfig()
            if add_source_safe_square_transform(cfg_pre_pd, xmin, ymin, xmax, ymax, 128):
