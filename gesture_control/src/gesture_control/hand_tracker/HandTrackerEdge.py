...
class HandTracker:
    ...
    def __init__(self, input_src=None,
        ...
        # Define data queues 
        if not self.laconic:
            # Retain enough frames to pair the delayed on-device inference result
            # with the camera frame that produced it.
            self.q_video = self.cam_video_out.createOutputQueue(
                maxSize=30 if self.solo else 1, blocking=False)
    ...
    def next_frame(self):
        ...
        # The manager output is delayed by palm/landmark inference. Read its
        # sequence first, then display the camera frame with that same sequence.
        result_buffer = self.q_manager_out.get()
        res = marshal.loads(result_buffer.getData())
        if self.laconic:
            video_frame = np.zeros((self.img_h, self.img_w, 3), dtype=np.uint8)
        else:
            result_sequence = result_buffer.getSequenceNum()
            if self.solo and result_sequence:
                while True:
                    in_video = self.q_video.get()
                    video_sequence = in_video.getSequenceNum()
                    if video_sequence >= result_sequence:
                        video_frame = in_video.getCvFrame()
                        break
            else:
                video_frame = self.q_video.get().getCvFrame()
