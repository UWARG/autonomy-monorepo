...
def test_float_vs_quat_zero_yaw():
    ...
    rf = target_relative_drone_setpoint(drone, t_pos, 0.0, 2.0)
    rq = target_relative_drone_setpoint(drone, t_pos, yaw_quat(0.0), 2.0)
...
def test_float_vs_quat_90deg():
    drone = identity_pose()
    t_pos = Vector3D(5.0, 0.0, 0.0)
    rf = target_relative_drone_setpoint(drone, t_pos, math.pi / 2, 2.0)
    rq = target_relative_drone_setpoint(drone, t_pos, yaw_quat(math.pi / 2), 2.0)
    assert vec_close(rf.position, rq.position)
    assert quat_close(rf.orientation, rq.orientation)
...
def test_float_vs_quat_180deg():
    drone = identity_pose()
    t_pos = Vector3D(5.0, 0.0, 0.0)
    rf = target_relative_drone_setpoint(drone, t_pos, math.pi, 2.0)
    rq = target_relative_drone_setpoint(drone, t_pos, yaw_quat(math.pi), 2.0)
    assert vec_close(rf.position, rq.position)
    assert quat_close(rf.orientation, rq.orientation)
...
def test_float_vs_quat_arbitrary_yaw():
    ...
    t_pos = Vector3D(5.0, 0.0, 0.0)
    yaw = 1.23
    rf = target_relative_drone_setpoint(drone, t_pos, yaw, 2.0)
    rq = target_relative_drone_setpoint(drone, t_pos, yaw_quat(yaw), 2.0)
    assert vec_close(rf.position, rq.position)
    assert quat_close(rf.orientation, rq.orientation)
...
def test_position_zero_yaw_positive_distance():
    ...
    result = target_relative_drone_setpoint(identity_pose(), Vector3D(5.0, 0.0, 0.0), 0.0, 2.0)
    assert vec_close(result.position, Vector3D(7.0, 0.0, 0.0))

def test_position_zero_yaw_zero_distance():
    # setpoint == target position
    result = target_relative_drone_setpoint(identity_pose(), Vector3D(3.0, 1.0, 2.0), 0.0, 0.0)
...
def test_position_90deg_yaw():
    ...
    result = target_relative_drone_setpoint(identity_pose(), Vector3D(0.0, 0.0, 0.0), math.pi / 2, 2.0)
...
def test_position_180deg_yaw():
    ...
    result = target_relative_drone_setpoint(identity_pose(), Vector3D(5.0, 0.0, 0.0), math.pi, 2.0)
...
def test_position_arbitrary_yaw():
    ...
    result = target_relative_drone_setpoint(identity_pose(), t_pos, yaw, dist)
...
def test_position_translated_target():
    result = target_relative_drone_setpoint(identity_pose(), Vector3D(100.0, 50.0, -10.0), 0.0, 5.0)
    assert vec_close(result.position, Vector3D(105.0, 50.0, -10.0))

def test_position_small_distance():
    dist = 1e-9
    result = target_relative_drone_setpoint(identity_pose(), Vector3D(0.0, 0.0, 0.0), 0.0, dist)
...
def test_frame_drone_yaw_90_position():
    ...
    result = target_relative_drone_setpoint(yaw_pose(math.pi / 2), Vector3D(5.0, 0.0, 0.0), 0.0, 2.0)
...
def test_frame_drone_yaw_180_position():
    ...
    result = target_relative_drone_setpoint(yaw_pose(math.pi), Vector3D(5.0, 0.0, 0.0), 0.0, 2.0)
    assert vec_close(result.position, Vector3D(-7.0, 0.0, 0.0))

def test_frame_translated_drone():
    # Drone at (3,0,0) identity. World setpoint = (7,0,0). Local = (4,0,0).
    result = target_relative_drone_setpoint(identity_pose(3.0, 0.0, 0.0), Vector3D(5.0, 0.0, 0.0), 0.0, 2.0)
...
def test_frame_translated_drone_and_target():
    ...
    # Local = (4,0,0).
    result = target_relative_drone_setpoint(identity_pose(1.0, 1.0, 0.0), Vector3D(4.0, 1.0, 0.0), 0.0, 1.0)
    assert vec_close(result.position, Vector3D(4.0, 0.0, 0.0))
...
def test_frame_rotation_preserves_magnitude():
    ...
    for drone_yaw in [0.0, math.pi / 2, math.pi, -math.pi / 2]:
        result = target_relative_drone_setpoint(yaw_pose(drone_yaw), t_pos, 0.0, 1.0)
...
def test_frame_arbitrary_drone_yaw_and_position():
    ...
    yaw_t = 1.1
    dist = 1.5
    result = target_relative_drone_setpoint(drone, t_pos, yaw_t, dist)
    sp_world = Vector3D(
...
def test_singular_forward_exactly_world_x():
    ...
    result = target_relative_drone_setpoint(identity_pose(), Vector3D(0.0, 0.0, 0.0), 0.0, 3.0)
    assert vec_close(result.position, Vector3D(3.0, 0.0, 0.0))

def test_singular_forward_exactly_negative_world_x():
    # yaw=π → forward=(-1,0,0). Must not crash.
    result = target_relative_drone_setpoint(identity_pose(), Vector3D(0.0, 0.0, 0.0), math.pi, 3.0)
    assert vec_close(result.position, Vector3D(-3.0, 0.0, 0.0))
...
def test_coincident_zero_distance():
    ...
    result = target_relative_drone_setpoint(drone, Vector3D(2.0, 2.0, 0.0), 0.0, 0.0)
...
def test_coincident_nonzero_distance():
    ...
    result = target_relative_drone_setpoint(drone, Vector3D(5.0, 5.0, 0.0), 0.0, 4.0)
    assert vec_close(result.position, Vector3D(4.0, 0.0, 0.0))